...
import type { MealDetail, MealItemSnapshot } from '@/core/application/manual-logging-ports';
...
const draftIdentity = (item: MealItemSnapshot) => ({ id: item.id, canonicalName: item.inputName, inputQuantity: unscaleDecimal(item.inputQuantityScaled), inputUnit: item.inputUnit, resolutionMethod: item.resolutionMethod });

export function MealDetailScreen({ mealId }: Readonly<{ mealId: string }>) {
  const router = useRouter(); const logging = useManualLogging(); const draftStore = useMealDraft();
  const [detail, setDetail] = useState<MealDetail | null>(null); const [state, setState] = useState<'loading' | 'ready' | 'missing' | 'error'>('loading'); const [error, setError] = useState<string | null>(null); const [working, setWorking] = useState(false); const [confirmDelete, setConfirmDelete] = useState(false);
  ...
  const beginEdit = async () => {
    if (detail === null || working) return;
    setWorking(true); setError(null);
    try {
      const items: MealDraftItem[] = [];
      for (const item of detail.items) {
        if (item.foodId === null || item.foodRevisionId === null) {
          items.push({ ...draftIdentity(item), foodId: `missing-${item.id}`, foodRevisionId: '', portionId: null, requiresReview: true, reviewReason: 'This saved item no longer links to a food. Remove it or add a replacement.' });
          continue;
        }
        try {
          const [food, portions] = await Promise.all([logging.food(item.foodId), logging.portions(item.foodId)]);
          const canonical = (canonicalUnits as readonly string[]).includes(item.inputUnit);
          const portion = canonical ? null : portions.find((candidate) => candidate.normalizedLabel === item.inputUnit) ?? null;
          const reason = food.state.archivedAt !== null ? 'This food is archived. Restore it or remove this item.' : food.revision.id !== item.foodRevisionId ? 'This food has a newer nutrition revision. Explicitly accept a current unit or portion.' : !canonical && portion === null ? 'The original portion is no longer available. Choose a current unit or portion.' : null;
          items.push({ ...draftIdentity(item), foodId: food.state.id, foodRevisionId: food.revision.id, canonicalName: food.state.canonicalName, portionId: portion?.id ?? null, requiresReview: reason !== null, reviewReason: reason });
        } catch {
          items.push({ ...draftIdentity(item), foodId: item.foodId, foodRevisionId: item.foodRevisionId, portionId: null, requiresReview: true, reviewReason: 'Current food details could not be loaded. Remove or replace this item.' });
        }
      }
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Meal could not be prepared for review.'); }
