...
export function MealDetailScreen({ mealId }: Readonly<{ mealId: string }>) {
  ...
  useEffect(() => { let active = true; void logging.mealDetail(mealId).then((value) => { if (active) { setDetail(value); setState('ready'); } }).catch((cause) => { if (active) setState(cause instanceof Error && cause.name === 'NotFoundError' ? 'missing' : 'error'); }); return () => { active = false; }; }, [logging, mealId]);
