...
export function ReviewScreen() {
  ...
  const save = async () => { if (saving || preview === null) return; setSaving(true); setError(null); try { const command = toMealCommand(draft); let mealId: string; if (draft.mode === 'create') mealId = await logging.createMeal(command); else if (draft.mealId !== null) { await logging.editMeal(draft.mealId, command); mealId = draft.mealId; } else throw new Error('Meal edit is missing its ID.'); draftStore.clear(); router.replace(`/meal-detail?mealId=${encodeURIComponent(mealId)}` as Href); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Meal could not be saved. Your draft is still available.'); } finally { setSaving(false); } };
